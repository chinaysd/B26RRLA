...
static unsigned char Mass_Flag;
...
void Mass_HandleRe(unsigned char Mass_Status)
{
	if(Mass_Status == Msg_MassOpen)
	{
		Mass_Flag = Mass_True;
	}
	else if(Mass_Status == Msg_MassClose)
	{
		Mass_Flag = Mass_False;
	}
}

void Mass_BackHandle(void)
{
	if(Mass_Flag)
	{
		Mass_Handle();
